...
angular.module( 'nzb.utils' )
	.controller( 'MusicSearchCtrl', function($scope, $log, $location, MusicSearchService) {
		...
		$scope.resultLimit = MusicSearchService.limit || 100;

		// Start with no results (or selected results) in the table.
		$scope.results = MusicSearchService.results;

		// Default the Selection to nothing
		$scope.selectedItems = [];


		$scope.searchText = MusicSearchService.searchText || "";
		$scope.searchType = findSearchType(MusicSearchService.searchType, $scope.searchTypes[0]);
		$scope.resultType = findResultType(MusicSearchService.resultType, $scope.resultTypes[0]);
		...
		$scope.doSearch = function() {
			...
			MusicSearchService.get()
				.success(function(response){
					...
					$scope.results = MusicSearchService.results = response.results;
